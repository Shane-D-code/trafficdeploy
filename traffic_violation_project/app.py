...
from violation_detector import detect_violations, draw_violations
...
def show_violation_table(violations):
    ...
    st.write(
        df[["Type", "Conf badge", "Plate"]].to_html(escape=False, index=False),
...
def display_violation_details(violation):
    if not violation:
        return
    with st.expander(f"🔍 Audit Trail - {violation['type']}"):
        st.write("**Detection Rules Applied:**")
        vtype = violation['type']
        if vtype == "NO HELMET":
            st.write("""
            - Rule: Rider detected without helmet
            - Logic: Checked IoU between rider upper body and helmet boxes
            - Threshold: IoU > 0.3 required for helmet detection
            """)
        elif vtype == "NO SEATBELT":
            st.write("""
            - Rule: Driver detected without seatbelt
            - Logic: Checked IoU between driver region and seatbelt boxes
            - Threshold: IoU > 0.1 required for seatbelt detection
            """)
        elif vtype == "TRIPLE RIDING":
            st.write("""
            - Rule: More than 2 riders detected on vehicle
            - Logic: Counted riders with IoU > 0.1 overlapping vehicle
            """)

        if violation.get('plate_text'):
            st.write("**OCR Details:**")
            st.write(f"- Plate Text: {violation['plate_text']}")
            st.write(f"- OCR Confidence: {violation.get('ocr_confidence', 'N/A')}")
            st.write(f"- Plate Validated: {'✅' if violation.get('plate_valid') else '❌'}")


def display_plate_with_confidence(plate_text, ocr_confidence, plate_valid):
    if not plate_text:
    ...
    if ocr_confidence and ocr_confidence > 0.8:
        color = "#22c55e"
        emoji = "✅"
    elif ocr_confidence and ocr_confidence > 0.5:
        color = "#f59e0b"
        emoji = "⚠️"
    else:
        color = "#ef4444"
    ...
    valid_text = "✅ Valid format" if plate_valid else "❌ Invalid format"
    conf_text = f"{ocr_confidence:.2%}" if ocr_confidence else "N/A"

    st.markdown(f"""
    <div style="border:2px solid {color};border-radius:10px;padding:10px;background-color:#f0f0f0;">
        <h3>{emoji} Plate: {plate_text}</h3>
        <p>OCR Confidence: {conf_text}</p>
        <p>{valid_text}</p>
    </div>
    """, unsafe_allow_html=True)
...
def draw_all_boxes(image, results):
    ...
    for i in range(len(boxes)):
        ...
        cv2.putText(
            img, label, (x1, max(y1 - 5, 15)), cv2.FONT_HERSHEY_SIMPLEX, 0.5, color, 2
        )
    return img
...
if page == PAGES[0]:
    ...
    if uploaded is not None:
        ...
        if not is_video:
            ...
            if detect_btn or st.session_state.processed:
                if detect_btn:
                    with st.spinner("Running detection..."):
                        try:
                            all_violations, all_detections = detect_violations(
                                tmp_path, enable_preprocessing=enable_preprocessing
                            )
                            filtered = filter_violations_by_threshold(
                                all_violations, conf_threshold
                            )
                            ...
                            st.session_state.annotated_img = cv2.cvtColor(
                ...
                if st.session_state.annotated_img is not None:
                    ...
                    if detections:
                        ...
                        vehicles = [
                            d for d in detections if d["class_name"] == "vehicle"
                        ]
                        helmets = [d for d in detections if d["class_name"] == "helmet"]
                        seatbelts = [
                            d for d in detections if d["class_name"] == "seatbelt"
                        ]
                    ...
                    if violations:
                        ...
                        st.markdown("### Violation Details & Audit Trail")
                        for i, v in enumerate(violations):
                            display_violation_details(v)
                        ...
                        st.info(
                            "No violations detected. Try lowering the confidence threshold in the sidebar."
                        )
            ...
            if process_vid:
                ...
                st.write(
                    f"Frames: {total_frames}  |  FPS: {fps_rate:.1f}  |  Duration: {duration:.1f}s"
                )
                all_violations = {}
                frame_count = 0
                processed = 0
                ...
                while True:
                    ...
                    if frame_count % 30 == 0:
                        ...
                        try:
                            ...
                            filtered = filter_violations_by_threshold(
                                frame_violations, conf_threshold
                            )
                            for v in filtered:
                            ...
                            import logging
                            logging.getLogger(__name__).error(f"Frame {frame_count} error: {e}")
                        finally:
                ...
                for k, v in all_violations.items():
                    v["avg_conf"] /= v["count"]
                summary = sorted(
                    all_violations.values(), key=lambda x: x["count"], reverse=True
                )
                        ...
                        rows.append(
                            {
                                "Type": s["type"],
                                "Plate": s["plate"] or "-",
                                "Frames with violation": s["count"],
                                "Avg confidence": f"{s['avg_conf']:.2f}",
                            }
                        )
                ...
                if st.button("Save Video Summary to Database"):
                    ...
                    for s in st.session_state.video_summary:
                        save_violation(
                            v_type=s["type"],
                            plate=s["plate"],
                            confidence=round(s["avg_conf"], 2),
                            image_path=None,
                        )
                        saved += 1
                    st.success(
                        f"Saved {saved} violation summary record(s) to database."
                    )
    ...
    if not all_rows:
        ...
        if plate_search:
            filtered = filtered[
                filtered["plate_text"].str.contains(
                    plate_search, na=False, case=False
                )
            ]
        ...
        with col_p2:
            st.markdown(
                f"<div style='text-align:center'>Page {page_num} of {total_pages} ({len(filtered)} records)</div>",
                unsafe_allow_html=True,
            )
        with col_p3:
    ...
    if not rows:
        ...
        with k3:
            plate_count = len(
                {r["plate_text"] for r in rows if r.get("plate_text")}
            )
        ...
        if by_date:
            line_df = pd.DataFrame(
                {"Date": list(by_date.keys()), "Count": list(by_date.values())}
            )
        ...
        plates = [r["plate_text"] for r in rows if r.get("plate_text")]
        if plates:
            plate_series = pd.Series(plates).value_counts().head(10)
            pie_df = pd.DataFrame(
                {"Plate": plate_series.index, "Count": plate_series.values}
            )
            try:
                ...
                fig = px.pie(
                    ...
                    names="Plate",
                    values="Count",
                    title="Top 10 Plates by Violation Count",
                ...
                try:
                    ...
                    labels = [
                        f"{r} ({c})" for r, c in zip(pie_df["Plate"], pie_df["Count"])
                    ]
                    ax.pie(
                        pie_df["Count"],
                        labels=labels,
        ...
        if total_violations > 0:
            ...
            seatbelt_compliance = (
                max(0, (1 - no_seatbelt_count / total_violations)) * 100
            )
            c1, c2 = st.columns(2)
            with c1:
