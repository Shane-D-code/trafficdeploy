...
def generate_pdf_report(violations, stats, output_path=None):
    ...
    elements.append(Paragraph(f"Total Violations: {stats.get('total', 0)}", styles['Normal']))
    elements.append(Paragraph(f"Vehicles Detected: {stats.get('total_vehicles', 0)}", styles['Normal']))
    elements.append(Spacer(1, 12))
    if violations:
...
if __name__ == '__main__':
    ...
    test_stats = {'total': 2, 'total_vehicles': 2, 'by_type': {'NO HELMET': 1, 'NO SEATBELT': 1}}
